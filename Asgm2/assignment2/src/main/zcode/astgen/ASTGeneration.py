...
class ASTGeneration(ZCodeVisitor):
    ...
    #     return None + self.visitNewlines(ctx.newlines()) if (ctx.newlines()) else None
   ...
   # expr_sign:		SUB_OPS expr_sign | expr_idx;												// (7)
    ...
    # elif_stmtTail:		elif_stmt elif_stmtTail | ;
    # Visit a parse tree produced by ZCodeParser#elif_stmtTail.
    # def visitElif_stmtTail(self, ctx: ZCodeParser.Elif_stmtTailContext):
    #     return ([self.visit(ctx.elif_stmt())] + self.visit(ctx.elif_stmtTail())) if (ctx.elif_stmtTail()) else []
    ...
    def visitParamList_prime(self, ctx:ZCodeParser.ParamList_primeContext):
        return [self.visit(ctx.param())] + (self.visit(ctx.paramList_prime()) if (ctx.paramList_prime()) else [])

    # param:			typ IDENTIFIER (LSB numList_prime RSB)?;
    # Visit a parse tree produced by ZCodeParser#param.
    def visitParam(self, ctx:ZCodeParser.ParamContext):
        typ = ArrayType(
            self.visit(ctx.numList_prime()), 
            self.visit(ctx.typ())) if (ctx.numList_prime()) else self.visit(ctx.typ()
        )
